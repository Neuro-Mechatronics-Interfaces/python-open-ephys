...
def compute_landmark_movement_signal(
    landmarks: np.ndarray, time_vector: np.ndarray, method: str = "velocity"
    ...
    if method == "velocity":
        ...
        raise ValueError(
            f"Unknown method: {method}. Use 'velocity', 'acceleration', or 'displacement'"
        )
...
def find_sync_offset(
    ...
    if emg_times.size < 2 or landmark_times.size < 2:
        raise ValueError(
            "EMG and landmark timelines must each contain at least two timestamps"
        )
    ...
    f_emg = interp1d(
        emg_times, emg_signal, kind="linear", fill_value=0, bounds_error=False
    )
    f_landmark = interp1d(
        landmark_times, landmark_signal, kind="linear", fill_value=0, bounds_error=False
    )
    ...
    # Normalize signals
    emg_resampled = (emg_resampled - np.mean(emg_resampled)) / (
        np.std(emg_resampled) + 1e-9
    )
    landmark_resampled = (landmark_resampled - np.mean(landmark_resampled)) / (
        np.std(landmark_resampled) + 1e-9
    )
