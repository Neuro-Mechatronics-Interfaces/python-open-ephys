...
class EMGPreprocessor:
    ...
    def __init__(
        self,
        fs: float = 2000.0,
        band: tuple[float, float] = (20.0, 498.0),
    ...
    def _ensure_initialized(self, C: int):
        ...
        if self.env_cut is not None:
            self._env_sos = butter(
                self.env_order, self.env_cut, btype="low", fs=self.fs, output="sos"
            )
        ...
        *,
        ...
        if cut is not None:
            ...
            if self._env_sos is not None and cut == self.env_cut:
                for ch in range(C):
                    y[ch, :], self._env_zi[:, ch, :] = sosfilt(
                        self._env_sos, y[ch, :], zi=self._env_zi[:, ch, :]
                    )
            else:
