...
def moving_average(
    signal: np.ndarray, window_size: int = 5, axis: int = 0
) -> np.ndarray:
    """
    ...
    if signal.ndim == 1:
        ...
        padded = np.pad(
            signal,
            [(pad_size, pad_size) if i == axis else (0, 0) for i in range(signal.ndim)],
            mode="edge",
        )
        kernel = np.ones(window_size) / window_size
        smoothed = np.apply_along_axis(
            lambda x: np.convolve(x, kernel, mode="valid"), axis, padded
        )
...
def lowpass_filter(
    signal: np.ndarray, cutoff: float, fs: float, order: int = 4, axis: int = 0
) -> np.ndarray:
    """
...
def savitzky_golay(
    signal: np.ndarray, window_length: int = 11, polyorder: int = 3, axis: int = 0
) -> np.ndarray:
    """
...
def gaussian_smooth(
    signal: np.ndarray, sigma: float = 2.0, axis: int = 0
) -> np.ndarray:
    """
...
def exponential_smooth(
    signal: np.ndarray, alpha: float = 0.3, axis: int = 0
) -> np.ndarray:
    """
...
def adaptive_smooth(
    ...
    velocity = np.pad(
        velocity,
        [(1, 0) if i == axis else (0, 0) for i in range(signal.ndim)],
        mode="edge",
    )
...
def smooth_predictions(
    ...
    method: Literal[
        "moving_average", "lowpass", "savgol", "gaussian", "exponential", "adaptive"
    ] = "savgol",
    fs: Optional[float] = None,
    **kwargs,
) -> np.ndarray:
    ...
    if method == "moving_average":
        ...
        return savitzky_golay(
            predictions, window_length=window_length, polyorder=polyorder
        )
