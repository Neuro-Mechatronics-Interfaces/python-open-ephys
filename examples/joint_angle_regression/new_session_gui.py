...
class OpenEphysMonitor:
    """Monitor for Open Ephys ZMQ streaming (adapted from MindRoveMonitor)."""
    ...
    def start(self):
        ...
        if ZMQClient is None:
            self.labels["status"].setText(
                f"Status: python-oephys missing ({OEPHYS_IMPORT_ERROR})"
            )
        ...
        try:
            ...
            self.labels["status"].setText("Status: Connecting...")
            self.labels["status"].setStyleSheet("color: #ffaa00; font-weight: bold;")

            self.client = ZMQClient(
                host_ip=self.zmq_host,
                data_port=str(self.zmq_port),
                buffer_seconds=30.0,
                expected_channel_count=self.expected_channels,
                auto_start=False,
                verbose=False,  # Suppress verbose logging
            )

            # Disable periodic index logging (suppress [IDX] spam)
            self.client.index_log_interval_s = float("inf")

            # Start the client (sockets now connecting)
            self.client.start()

            # Wait for first data packet to arrive
            if not self.client.ready_event.wait(timeout=5.0):
                self.client.stop()
                raise RuntimeError(
                    f"No data from tcp://{self.zmq_host}:{self.zmq_port} (timeout after 5s).\n"
                    "** Press 'Run' in Open Ephys GUI to start streaming **"
                )

            self.streaming = True
            self.last_update = _clock()
            if abs(self.client.fs - self.expected_fs) > 1.0:
                print(
                    f"[INFO] Detected fs {self.client.fs} Hz (expected {self.expected_fs} Hz)"
                )
                self.expected_fs = self.client.fs

            # Start IMU client if enabled
            if self.imu_enabled and SLEEVEIMU_AVAILABLE and SleeveIMUClient:
                try:
                    ...
                    if self.imu_client.wait_connected(timeout=3.0):
                        print(
                            f"[INFO] IMU connected: {self.imu_transport}@{self.imu_host}:{self.imu_port}"
                        )
                        self.labels["imu"].setText("IMU: Connected")
                    else:
                        print("[WARN] IMU connection timeout")
                        self.labels["imu"].setText("IMU: Timeout")
                except Exception as imu_exc:
                    print(f"[WARN] IMU connection failed: {imu_exc}")
                    self.labels["imu"].setText("IMU: Error")
                    self.imu_client = None

            self.labels["status"].setText("Status: Streaming")
            self.labels["status"].setStyleSheet("color: #44ff44; font-weight: bold;")
        except Exception as exc:
            error_text = str(exc).replace("\n", " ")
            self.labels["status"].setText(f"Status: Error - {error_text}")
            self.labels["status"].setStyleSheet("color: #ff4444; font-weight: bold;")
            print(f"\n[CONNECTION ERROR]\n{exc}\n")
            self.streaming = False
            if self.client:
    ...
    def stop(self):
        ...
        self.streaming = False
        self.client = None
        self.labels["status"].setText("Status: Disconnected")
        self.labels["status"].setStyleSheet("color: #ff6666; font-weight: bold;")
        if "imu" in self.labels:
            self.labels["imu"].setText("IMU σ: N/A (Open Ephys)")

    def poll(self, auto_retry):
        if not self.streaming or self.client is None:
            if auto_retry and not self.user_stopped:
                now = _clock()
            ...
            return
        try:
            # Update poll rate regardless of data availability
            now = _clock()
            dt = max(now - self.last_update, 1e-6)
            self.labels["rate"].setText(f"Update rate: {1.0 / dt:.1f} Hz")
            self.last_update = now

            # Estimate buffer size
            buffer_samples = sum(
                len(buf) for buf in self.client.buffers[: self.expected_channels]
            )
            self.labels["samples"].setText(f"Samples buffered: {buffer_samples}")

            # Get latest data for display (non-destructive)
            # Use get_latest() instead of drain_new() for GUI updates
            if buffer_samples > 0:
                try:
                    # Get last ~100ms of data for display
                    n_display = max(10, int(self.client.fs * 0.1))  # 100ms worth
                    Y_data, t_abs = self.client.get_latest(n_display)

                    if Y_data is not None and Y_data.shape[1] > 0:
                        # Y_data is (channels, samples) - select only EMG channels
                        emg = Y_data[: self.expected_channels, :]

                        # Calculate metrics for display
                        emg_std = float(np.std(emg))
                        self.last_emg_rms = float(np.sqrt(np.mean(emg**2)))
                        self.labels["emg_rms"].setText(
                            f"EMG RMS: {self.last_emg_rms:.3f}"
                        )
                        self.labels["emg_std"].setText(f"EMG σ: {emg_std:.3f}")

                        # For recording, cache the latest chunk using drain_new()
                        # This ensures we don't record duplicate data
                        t_rec, Y_rec = self.client.drain_new()
                        if Y_rec is not None and Y_rec.shape[1] > 0:
                            emg_rec = Y_rec[: self.expected_channels, :]
                            self.last_emg_chunk = emg_rec.T  # (samples, channels)
                            fs = self.client.fs
                            if t_rec is not None and len(t_rec) > 0:
                                self.last_emg_ts = t_rec[: len(self.last_emg_chunk)]
                            else:
                                t_end = _clock()
                            ...
                            # Collect IMU data if client is running
                            if self.imu_client and self.imu_client.is_running():
                                rpy = self.imu_client.get_rpy_deg()
                                if rpy:
                                    r, p, y = rpy
                                    n_samples = self.last_emg_chunk.shape[0]
                                    imu_row = np.array(
                                        [r, p, y, 0, 0, 0, 0, 0, 0], dtype=np.float32
                                    )
                                    self.last_imu_chunk = np.tile(
                                        imu_row, (n_samples, 1)
                                    )
                                    self.last_imu_ts = self.last_emg_ts

                                    self.labels["imu"].setText(
                                        f"IMU: R{r:.1f}° P{p:.1f}° Y{y:.1f}°"
                                    )
                                    self.labels["mag"].setText("Mag: RPY mode")
                                else:
                                    self.labels["imu"].setText("IMU: No data")
                                    self.labels["mag"].setText("Mag: N/A")
                            else:
                                # No IMU client or not running
                                self.labels["imu"].setText("IMU: N/A (Open Ephys)")
                                self.labels["mag"].setText("Mag: N/A")
                except Exception as e:
                    # If get_latest fails, fall back to showing buffer status only
                    if "not ready" not in str(e).lower():
                        print(f"[POLL] Error getting data: {e}")
        except Exception as exc:
            print(f"[POLL ERROR] {exc}")
            import traceback

            traceback.print_exc()
            self.labels["status"].setText(f"Status: Error - {exc}")
            self.labels["status"].setStyleSheet("color: #ff4444; font-weight: bold;")
            self.stop()
...
class FlowDiagram(QWidget):
    def __init__(self, parent=None):
        ...
        self.blocks["sync"].set_size_profile(
            compact_width=70, full_width=90, min_height=84
        )
        self.blocks["buffer"].set_size_profile(compact_width=100, full_width=135)
        self.blocks["features"].set_size_profile(compact_width=105, full_width=140)
        self.blocks["infer"].set_size_profile(compact_width=75, full_width=100)
        self.blocks["compare"].set_size_profile(compact_width=90, full_width=120)
        self.blocks["train"].set_size_profile(compact_width=85, full_width=110)
        self.blocks["saved"].set_size_profile(compact_width=85, full_width=110)
...
class SessionConsole(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Joint Angle Regression Console (Open Ephys Edition)")
        ...
        # Create monitor with connection params (will be set from GUI fields before start)
        self.monitor = OpenEphysMonitor(
            labels={},  # Will be set after _build_oephys_panel
            zmq_host="127.0.0.1",
            zmq_port=5556,
            expected_fs=5000.0,
            expected_channels=8,
        )
        # Update labels after panel is built
        self.monitor.labels = self.oephys_labels
        self.timer = QTimer()
    ...
    def _init_ui(self):
        ...
        top_row.addWidget(self._build_oephys_panel(), stretch=0)
    ...
    def _build_oephys_panel(self):
        group = QGroupBox("EMG Acquisition (Open Ephys ZMQ)")
        layout = QVBoxLayout(group)

        # Connection settings
        conn_layout = QGridLayout()
        conn_layout.addWidget(QLabel("ZMQ Host:"), 0, 0)
        self.zmq_host = QLineEdit("127.0.0.1")
        self.zmq_host.setMaximumWidth(120)
        conn_layout.addWidget(self.zmq_host, 0, 1)

        conn_layout.addWidget(QLabel("Port:"), 0, 2)
        self.zmq_port = QSpinBox()
        self.zmq_port.setRange(1024, 65535)
        self.zmq_port.setValue(5556)
        self.zmq_port.setMaximumWidth(80)
        conn_layout.addWidget(self.zmq_port, 0, 3)

        conn_layout.addWidget(QLabel("fs (Hz):"), 1, 0)
        self.oephys_fs = QLineEdit("5000")
        self.oephys_fs.setMaximumWidth(80)
        conn_layout.addWidget(self.oephys_fs, 1, 1)

        conn_layout.addWidget(QLabel("Channels:"), 1, 2)
        self.oephys_channels = QSpinBox()
        self.oephys_channels.setRange(1, 256)
        self.oephys_channels.setValue(8)
        self.oephys_channels.setMaximumWidth(60)
        conn_layout.addWidget(self.oephys_channels, 1, 3)

        layout.addLayout(conn_layout)

        # IMU Connection settings (collapsible)
        self.imu_enable_checkbox = QCheckBox("Enable Sleeve IMU")
        self.imu_enable_checkbox.setChecked(False)
        if not SLEEVEIMU_AVAILABLE:
            self.imu_enable_checkbox.setEnabled(False)
            self.imu_enable_checkbox.setText("Enable Sleeve IMU (not available)")
        layout.addWidget(self.imu_enable_checkbox)

        imu_layout = QGridLayout()
        imu_layout.addWidget(QLabel("IMU Host:"), 0, 0)
        self.imu_host_edit = QLineEdit("192.168.4.1")
        self.imu_host_edit.setMaximumWidth(120)
        imu_layout.addWidget(self.imu_host_edit, 0, 1)

        imu_layout.addWidget(QLabel("Port:"), 0, 2)
        self.imu_port_spin = QSpinBox()
        self.imu_port_spin.setRange(1024, 65535)
        self.imu_port_spin.setValue(5555)
        self.imu_port_spin.setMaximumWidth(80)
        imu_layout.addWidget(self.imu_port_spin, 0, 3)

        imu_layout.addWidget(QLabel("Transport:"), 1, 0)
        self.imu_transport_combo = QComboBox()
        self.imu_transport_combo.addItems(["UDP", "TCP"])
        self.imu_transport_combo.setCurrentText("UDP")
        self.imu_transport_combo.setMaximumWidth(80)
        imu_layout.addWidget(self.imu_transport_combo, 1, 1)

        layout.addLayout(imu_layout)

        reminder = QLabel("Ensure Open Ephys GUI is running with ZMQ Interface plugin")
        reminder.setStyleSheet("color: #ffaa00; font-size: 10px;")
        reminder.setWordWrap(True)

        status = QLabel("Disconnected")
        status.setStyleSheet("color: #ff6666; font-weight: bold;")
        samples = QLabel("Buf: 0")
        emg_rms = QLabel("EMG RMS: N/A")
        emg_std = QLabel("EMG σ: N/A")
        imu = QLabel("IMU σ: N/A")
        mag = QLabel("Mag σ: N/A")
        rate = QLabel("Rate: N/A")

        btn_row = QHBoxLayout()
        btn_start = QPushButton("Connect")
        btn_stop = QPushButton("Disconnect")
        btn_stop.setEnabled(False)
        btn_start.clicked.connect(lambda: self._on_oephys_start(btn_start, btn_stop))
        btn_stop.clicked.connect(lambda: self._on_oephys_stop(btn_start, btn_stop))
        btn_row.addWidget(btn_start)
        btn_row.addWidget(btn_stop)

        self.auto_retry = QCheckBox("Auto-retry connection")
        self.auto_retry.setChecked(True)

        layout.addWidget(reminder)
        layout.addWidget(status)
        layout.addWidget(samples)
        layout.addWidget(emg_rms)
        layout.addWidget(emg_std)
    ...
    def _build_lsl_panel(self):
        ...
        self.angle_stream_name.setMinimumWidth(140)
        self.angle_stream_name.setMaximumWidth(200)
        self.marker_stream_name.setMinimumWidth(140)
        self.marker_stream_name.setMaximumWidth(200)

        self.angle_status = QLabel("Angles")
        self.marker_status = QLabel("Markers")
        self.last_angle_label = QLabel("Last angle: N/A")
        ...
        led_row = QHBoxLayout()
        led_row.addWidget(self.mr_led)
        led_row.addWidget(QLabel("EMG (Open Ephys)"))
        led_row.addSpacing(12)
        led_row.addWidget(self.angle_led)
        led_row.addWidget(QLabel("Angles (LSL)"))
        ...
        status_grid.addWidget(self.angle_status, 0, 0)
        status_grid.addWidget(self.last_angle_label, 0, 1)
        ...
        marker_grid.addWidget(self.marker_status, 0, 0)
        marker_grid.addWidget(self.ready_status, 0, 1)
        layout.addLayout(marker_grid)
        layout.addLayout(led_row)
        led_note = QLabel(
            "LEDs indicate EMG (Open Ephys) and Angles (LSL) availability."
        )
        led_note.setStyleSheet("color: #9aa3ad;")
        layout.addWidget(led_note)
        layout.addWidget(self.arm_button)

        btn_rescan = QPushButton("Rescan LSL")
        btn_rescan.clicked.connect(self._rescan_lsl)
    ...
    def _build_live_panel(self):
        ...
        status_grid.addWidget(rec_widget, 0, 0)
        status_grid.addWidget(prompts_widget, 0, 1)
        status_grid.addWidget(self.record_windows_label, 1, 0, 1, 1)
        status_grid.addWidget(self.emg_buf_label, 1, 1, 1, 1)
        layout.addLayout(status_grid)
        self.emg_rms_label = QLabel("EMG RMS: N/A")
        self.angle_seen_label = QLabel("Angles: 0")
        self.angle_age_label = QLabel("Age: N/A")
        metrics_grid = QGridLayout()
        metrics_grid.setHorizontalSpacing(8)
        metrics_grid.addWidget(self.emg_rms_label, 0, 0)
        metrics_grid.addWidget(self.angle_seen_label, 0, 1)
        metrics_grid.addWidget(self.angle_age_label, 1, 0)
    ...
    def _poll_all(self):
        self.monitor.poll(self.auto_retry.isChecked())
        self._check_lsl(
    ...
    def _check_lsl(self, stream_name, label, kind):
        if not HAS_LSL or resolve_byprop is None:
            label.setText(f"{kind}: pylsl not available")
            return
        if not stream_name:
            label.setText(f"{kind}: (name empty)")
            return
        try:
            streams = resolve_byprop("name", stream_name, timeout=0.1)
            if streams:
                label.setText(f"{kind}: OK ({stream_name})")
            else:
                label.setText(f"{kind}: Not found ({stream_name})")
        except Exception as exc:
            label.setText(f"{kind}: Error ({exc})")
    ...
    def _rescan_lsl(self):
        if not HAS_LSL or resolve_byprop is None:
            ...
            return
        try:
            streams = resolve_byprop("type", "MOCAP", timeout=1.0)
            names = [s.name() for s in streams]
            self._append_log(f"[LSL] MOCAP streams: {names}")
        except Exception as exc:
    ...
    def _poll_angle_stream(self):
        if not HAS_LSL or StreamInlet is None:
            return
        if "OK" not in self.angle_status.text():
            self.angle_inlet = None
            return
        if self.angle_inlet is None:
            try:
                streams = resolve_byprop(
                    "name", self.angle_stream_name.text().strip(), timeout=0.1
                )
                if streams:
        ...
        self.angle_seen_label.setText(f"Angles: {self.angle_count}")
        self.angle_age_label.setText(
            f"Angles age: {age:.2f}s" if age is not None else "Angles age: N/A"
        )
        self.last_angle_label.setText(
            f"Last angle: {age:.2f}s ago" if age is not None else "Last angle: N/A"
        )

    def _poll_marker_stream(self):
        if not HAS_LSL or StreamInlet is None:
            return
        if "OK" not in self.marker_status.text():
            self.marker_inlet = None
            return
        if self.marker_inlet is None:
            try:
                streams = resolve_byprop(
                    "name", self.marker_stream_name.text().strip(), timeout=0.1
                )
                if streams:
    ...
    def _update_ready_state(self):
        mr_ok = self.monitor.streaming
        ang_ok = "OK" in self.angle_status.text()
        ...
        self.mr_led.setStyleSheet("color: #44ff44;" if mr_ok else "color: #ff6666;")
        self.angle_led.setStyleSheet("color: #44ff44;" if ang_ok else "color: #ff6666;")
        if self.record_start_btn:
            self.record_start_btn.setEnabled(ready)
        if self.monitor.last_emg_rms:
            self.emg_rms_label.setText(f"EMG RMS: {self.monitor.last_emg_rms:.3f}")
        self._update_flow_status()

    def _update_flow_status(self):
        if not self.flow_blocks:
            return
        emg_ok = self.monitor.streaming and self.monitor.last_emg_chunk is not None
        emg_detail = ""
        if emg_ok and self.monitor.last_emg_chunk is not None:
            emg_detail = f"RMS {self.monitor.last_emg_rms:.2f} | buf {self.monitor.last_emg_chunk.shape[0]}"
        ...
        hand_ok = "OK" in self.angle_status.text() and self.angle_count > 0
        ...
        if "imu" in self.flow_blocks:
            imu_ok = (
                self.monitor.imu_client
                and self.monitor.imu_client.is_running()
                and self.monitor.last_imu_chunk is not None
            )
            imu_detail = ""
            if imu_ok and self.monitor.last_imu_chunk is not None:
                # Extract RPY from IMU data (first 3 columns)
                r, p, y = self.monitor.last_imu_chunk[0, :3]
                imu_detail = f"RPY: ({r:.1f}, {p:.1f}, {y:.1f})°"
            elif self.monitor.imu_enabled:
                imu_detail = "Enabled but no data"
            else:
                imu_detail = "Disabled"

            self.flow_blocks["imu"].set_state(
                True if imu_ok else (None if not self.monitor.imu_enabled else False),
                "IMU: streaming"
                if imu_ok
                else (
                    "IMU: disconnected" if self.monitor.imu_enabled else "IMU: disabled"
                ),
                imu_detail,
            )
            self.flow_blocks["imu"].setToolTip(f"Sleeve IMU orientation\n{imu_detail}")
    ...
    def _on_oephys_start(self, btn_start, btn_stop):
        # Update monitor settings from GUI
        self.monitor.zmq_host = self.zmq_host.text()
        self.monitor.zmq_port = int(self.zmq_port.value())
        self.monitor.expected_fs = float(self.oephys_fs.text() or 5000.0)
        self.monitor.expected_channels = int(self.oephys_channels.value())
        # Update IMU settings
        self.monitor.imu_enabled = self.imu_enable_checkbox.isChecked()
        self.monitor.imu_host = self.imu_host_edit.text()
        self.monitor.imu_port = int(self.imu_port_spin.value())
        self.monitor.imu_transport = self.imu_transport_combo.currentText()
        self.monitor.start()
        btn_start.setEnabled(False)
        btn_stop.setEnabled(True)
        # Force immediate flow diagram update
        self._update_flow_status()

    def _on_oephys_stop(self, btn_start, btn_stop):
        self.monitor.stop()
    ...
    def _start_task(self):
        ...
        ready = (
            self.monitor.streaming
            and ("OK" in self.angle_status.text())
            and self.arm_button.isChecked()
        )
        if not ready:
            self._append_log(
                "[task] Not ready: need Open Ephys streaming + LSL Angles OK + Arm Recording."
            )
            return
    ...
    def _start_record(self, btn_start, btn_stop):
        ...
        if not (
            ...
            self._append_log(
                "[WARN] Not ready: need Open Ephys streaming, LSL Angles OK, and Arm Recording enabled."
            )
            return
        if get_target_keys:
    ...
    def _start_compare_gui(self, btn_start, btn_stop):
        if self.compare_active:
            return
        if not self.monitor.streaming:
            self._append_log("[compare] Open Ephys not streaming; connect first.")
            return
        if "OK" not in self.angle_status.text():
    ...
    def _record_tick(self):
        if not self.recording_active:
            return
        if not self.monitor.streaming or self.monitor.board is None:
            self._append_log("[record] Open Ephys stream not active; stopping.")
            self._finish_record(save=True)
            return
